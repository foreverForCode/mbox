...
Toast.install = function (Vue, options) {
    ...
    Vue.prototype.$toast = function (tips, type) {
        console.log('tips: '+tips,'type: '+type);
        var curType = type ? type : opt.defaultType;
        var wordWrap = opt.wordWrap ? 'lx-word-wrap' : '';  // lx-word-wrap
        var domWidth = opt.width ? 'style="width: ' + opt.width + '"' : '';
        // var tmp = '<div v-show="show" :class="[type,typess]" class="lx-toast ' + wordWrap + '" ' + domWidth + '><div class="title"></div>{{tip}}</div>';
        var tmp = `
            <div v-show="show" :class="[type,typess]" :style = "domWidth">
                <div class="title"></div>
