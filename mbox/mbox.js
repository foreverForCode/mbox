...
; (function (root, factory) {
    var mBox = factory(root);
    if (typeof define === 'function' && define.amd) {
        // AMD
        define('mBox', function () { return mBox; });
    } else if (typeof exports === 'object') {
        // Node.js
        module.exports = mBox;
    } else {
        // Browser globals
        root.mBox = $M = jemBox = mbox = mBox;
    }
})(this, function () {
    var doc = document,
        JM = { timer: {}, endCall: {} }, 
    ...
    JM.goCss = function (elem, attr) {
        var val, cssNumber = { 'column-count': 1, 'columns': 1, 'box-flex': 1, 'line-clamp': 1, 'font-weight': 1, 'opacity': 1, 'z-index': 1, 'zoom': 1 },
    ...
    jefn.viewInit = function () {
        ...
        //判断是否有按钮参数，创建按钮
        var btnDiv = function () {
            var yesArr = opts.yesCall, noArr = opts.noCall, footCss, btnDom;
            //判断按钮对象是否为空
        ...
        JM.goCss(modDiv, { "pointer-events": "none", "z-index": opts.zIndex });
        //设置遮罩层
        maskDiv.setAttribute("id", "jemmask" + cellidx);
        maskDiv.classList.add("jemboxmask");
        JM.goCss(maskDiv, { "background-color": opts.maskColor, "z-index": opts.zIndex - 5 });
        //弹层模板元素
        modDiv.innerHTML =
        ...
        var elem = that.elem = $QM("#" + that.id)[0],
            setWH = { "width": opts.width != "" ? opts.width : "", "height": opts.height != "" ? opts.height : "" },
            conStyle = Object.assign({ "pointer-events": "auto", "background-color": "#fff", "border-radius": opts.radius }, opts.conStyle);
    ...
    jefn.contype = function (opts) {
        //设置显示的类型
        var that = this, ctype, tid = "#" + that.id, msg = opts.content, conCell = $QM(".jemboxmcont", tid)[0];
        ...
        if (typeof msg === "string") {
            ...
            if (JM.goCss(msg, "display") == "none") JM.goCss(msg, { "display": "block" });
    ...
    jefn.action = function (opts) {
        var that = this, tid = "#" + that.id, yesArr = opts.yesCall, noArr = opts.noCall;
        //自动关闭
        if (opts.time) {
    ...
    function backSitu(elem, prev, next, parent, isShow) {
        JM.goCss(elem.firstChild, { "display": isShow == "block" ? "block" : "none" });
    ...
    mBox.toast = function (options) {
        var conHtml = "", asscon = { "background-color": "rgba(0,0,0,0.80)", "color": "#fff" },
            opts = Object.assign({ width: '', padding: "20px 10px", content: "", conStyle: {}, type: "", time: 0, success: null }, options);
        //判断类型
        switch (opts.type) {
            case "none": conHtml = "<div class='jemtc'>" + opts.content + "</div>"; break;
            case "success": conHtml = "<div class='success'></div><div class='jemtc'>" + opts.content + "</div>"; break;
            case "error": conHtml = "<div class='error'></div><div class='jemtc'>" + opts.content + "</div>"; break;
            case "loading": conHtml = "<div class='loading'><span></span></div><div class='jemtc'>" + opts.content + "</div>"; break;
        }
        return mBox({
            cell: opts.cell,
            width: opts.width, padding: opts.padding,
            mask: false, time: opts.time, content: conHtml,
    ...
    mBox.alert = function (text) {
        return mBox({
            ...
            content: text, yesCall: { name: "确定", fun: function (idx) { mBox.close(idx) } }
        });
    };
    //通知提示
    mBox.notice = function (options) {
        var opts = Object.assign({ content: "", time: 5, conStyle: {} }, options),
            asscon = { position: "fixed", top: 0, left: 0, "background-color": "rgba(0,0,0,0.80)", "color": "#fff" };
        return mBox({
            cell: opts.cell,
            width: '100%', padding: "8px 10px", time: opts.time, radius: "0px",
            content: "<div class='jemtc'>" + opts.content + "</div>", mask: false,
            conStyle: Object.assign(asscon, opts.conStyle)
        });
    };
    //弹出式菜单
    mBox.actionSheet = function (options) {
        //合并默认的API
        var opts = Object.assign({ title: "", closeBtn: [false, 1], content: "", skins: "", time: 0, conStyle: {}, success: null, yesCall: null, noCall: { name: "取消" } }, options);
        //判断向上弹出层的效果类型
        if (opts.skins == "websheet") {
            var childWidth = '100%', bgColor = { "background-color": "#fff" };
        } else if (opts.skins == "itemsheet") {
            var childWidth = '95%', bgColor = { "background-color": "rgba(255,255,255,0)" };
        }
        return mBox({
            cell: opts.cell,
            width: childWidth, padding: "0px", time: opts.time, title: opts.title,
            closeBtn: opts.closeBtn, skin: opts.skins, radius: "", content: opts.content,
            conStyle: Object.assign(bgColor, opts.conStyle), success: opts.success,
            yesCall: opts.yesCall, noCall: opts.noCall
        });
    };
    //关闭单个层
    mBox.close = function (mid) {
        var mCell = $QM("#jembox" + mid)[0], maskCell = $QM("#jemmask" + mid),
